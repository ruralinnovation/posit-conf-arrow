...
rural_places_geometry <- rural_places_ddb$geometry  # <= TODO: What type is this???
...
for (r in c(1:length(rural_places_properties))) {
  for (c in c(1:length(rural_places_properties[[r]]))) {
      ...
      if (length(unlist(rural_places_properties[[r]])) > 61) {
...
rural_places <- arrow::open_dataset("data/rural_places") # <= available at s3://cori-risi-apps/examples/who-wins-b2s/rural_places

rural_places |>
  mutate(prop_score = as.double(prop_score)) |>
  summarize(
